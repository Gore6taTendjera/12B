let image = new Image();
image.src="images/Screenshot_230.webp";
image.addEventListener('load', function(){
    ...
    class Particle {
        ...
        update () {
            ...
            this.y -= movement + Math.cos(this.angle) * 0;
            this.x += Math.cos(this.angle) * 0;
            if (this.y <= 0) {
        ...
        draw(){
            ctx.beginPath();
            ctx.fillStyle = 'black';
            if (this.y > canvas.height - this.size * 14) ctx.globalAlpha = 0;
            if (grid[this.position1]){
                if (grid[this.position1][this.position2]){
